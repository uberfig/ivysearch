...
use ivysearch::{
    ...
    indexing::{
        index_store::{IndexSharable, IndexStore},
...
async fn main() -> std::io::Result<()> {
    ...
    let wiki_index = WikiSharable::get().await;
    let index_info = IndexInfo::get().await;
    let stale = index_info.is_stale();

    let mut crawlstack: Vec<StackElem> = Vec::new();
    for site in &sites.sites {
        crawlstack.push(StackElem {
            depth: index_info.crawl_depth,
            site_depth: index_info.site_depth,
            page: site.clone(),
        });
    }
    ...
    if stale && index_info.run_crawler {
        ...
        for _ in 0..index_info.num_of_runners {
            ...
            actix_rt::spawn(async move {
                ...
                crawl(word_index.clone(), crawlstack.clone()).await;
                println!("finished crawling");

                let remain = active_threads.fetch_sub(1, std::sync::atomic::Ordering::SeqCst) - 1;
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .app_data(Data::new(wiki_index.clone()))
